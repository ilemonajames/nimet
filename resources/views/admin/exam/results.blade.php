...
@section('breadcrumb')
    ...
    'crumbs'=>[
            ...
            route('admin.exam.index')=>__lang('exams'),
            '#'=>isset($pageTitle)?$pageTitle:''
        ]])
@endsection
@section('search-form')
    <form class="form-inline mr-auto" method="get" action="{{ adminUrl(array('controller'=>'exam','action'=>'results','id'=>$row->id)) }}">
    ...
    <div >
        ...
        <div class="dropdown d-inline mr-2">
            ...
            <div class="dropdown-menu wide-btn">
                <a class="dropdown-item" href="{{ adminUrl(['controller'=>'exam','action'=>'exportresult','id'=>$row->id]) }}?type=pass&start={{ $start }}&end={{ $end }}" ><i class="fa fa-thumbs-up"></i> {{ __lang('export-passed') }}</a>
                <a class="dropdown-item"  href="{{ adminUrl(['controller'=>'exam','action'=>'exportresult','id'=>$row->id]) }}?type=fail&start={{ $start }}&end={{ $end }}"><i class="fa fa-thumbs-down"></i> {{ __lang('export-failed') }}</a>
        ...
        <div class="collapse" id="collapseFilter">
            <div class="card card-body">
                <form id="filterform"    role="form"  method="get" action="{{ adminUrl(array('controller'=>'exam','action'=>'results','id'=>$row->id)) }}">
                    <div class="row">
                        <div class="col-md-3">
                ...
                <table class="table table-hover">
                    ...
                    @php foreach($paginator as $row):  @endphp
                        <tr>
                            ...
                            <td class="text-right">
                                 <a onclick="openModal('{{ $row->first_name }} {{ $row->last_name }}','{{ adminUrl(array('controller'=>'exam','action'=>'examresult','id'=>$row->id)) }}')"  href="javascript:;" class="btn btn-xs btn-primary btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="{{ __lang('view-result') }}"><i class="fa fa-eye"></i></a>
                                <a onclick="return confirm('{{__lang('delete-confirm')}}')" href="{{ adminUrl(array('controller'=>'exam','action'=>'deleteresult','id'=>$row->id)) }}"  class="btn btn-xs btn-primary btn-equal" data-toggle="tooltip" data-placement="top" data-original-title="{{__lang('delete')}}"><i class="fa fa-trash"></i></a>
                            </td>
                        </tr>
