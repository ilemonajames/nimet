...
<div class="container">
    ...
    <table class="table table-striped">
        ...
        @foreach($exams as $test)
            <tr>
                <td>{{ $test->name }}</td>
                <td>{{ $test->passmark }}%</td>
                <td>     @php  $result =$test->studentExams()->where('student_id',$student->id)->orderBy('score','desc')->first()  @endphp
                    @if($result)
                ...
                <td>
                    @if($result)
                    {{ $examGradeTable->getGrade($result->score) }}
                    @endif
    ...
    <h4>Total</h4>
    <table class="table table-striped">
        @php  $stats = $controller->getStudentExamsStats($student->id);  @endphp
        <tr>
        ...
        <tr>
            ...
            <td>{{ $examGradeTable->getGrade($stats['average']) }}</td>
        </tr>
    </table>
