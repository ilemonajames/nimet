...
<div class="card"   >
    ...
    <div class="card-body">
            ...
            @php  foreach($paginator as $row):  @endphp
                <tr>
                    ...
                    @php  if(!$studentTest->hasTest($row->test_id,$id) || !empty($row->allow_multiple)):  @endphp
                        <a href="{{  route('aptitude.taketest',array('id'=>$row->test_id,'myid'=>$id)) }}" class="btn btn-primary " ><i class="fa fa-play"></i> {{  __lang('Take Test')  }}</a>
                    @php  endif;  @endphp

                        @php  if($studentTest->hasTest($row->test_id,$id) && $row->show_result==1):  @endphp
                            <a href="{{  route('aptitude.testresults',array( 'id'=>$row->test_id,'myid'=>$id)) }}" class="btn btn-success " ><i class="fa fa-list-ul"></i> {{  __lang('Your Results')  }}</a>
                        @php  endif;  @endphp
        ...
        // the paginator object
            ...
            route('aptitude.index',array('id'=>$id))
        );
         @endphp
    </div>
