...
class StudentTable extends BaseTable {
	...
	public function getStudents($paginated=false,$filter=null)
	{
        ...
        $select->join($this->getPrefix().'users',$this->tableName.'.user_id='.$this->getPrefix().'users.id',['name','email','picture','last_name','session','semester','payment_status']);
		if(isset($filter))
		{
	...
	public function getStudentSemester($id)
	{
        return $this->getSemesterRecord($id);
	}
    ...
    {
        $select = new Select($this->tableName);
        $select->join($this->getPrefix().'users',$this->tableName.'.user_id='.$this->getPrefix().'users.id',['name','email','picture','last_name','enabled','session','semester','olevel_result']);
        $select->where([$this->tableName.'.id'=>$id]);
        $row = $this->tableGateway->selectWith($select)->current();
        return $row;
    }

	public function getSessionRecord($id)
    {
        $select = new Select($this->getPrefix().'users');
        $select->join('academic_sessions','academic_sessions.id='.$this->getPrefix().'users.session',['session_name']);
        $select->where(['users.id'=>$id]);
        $row = $this->tableGateway->selectWith($select)->current();
        return $row;
    }

	public function getSemesterRecord($id)
    {
        $select = new Select($this->getPrefix().'users');
        $select->join('semesters','semesters.id='.$this->getPrefix().'users.semester',['semester_name']);
        $select->where(['users.id'=>$id]);
		$row = $this->tableGateway->selectWith($select)->current();
        return $row;
    }
