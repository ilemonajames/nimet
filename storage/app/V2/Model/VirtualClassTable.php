...
class VirtualClassTable extends BaseTable {

    protected $tableName = 'virtual_class';
    ...
    public function getPaginatedRecords($paginated=false,$id=null,$activeOnly=false,$filter=null,$group=null,$order=null,$type=null,$futureOnly=false,$payment=null)
    {
        ...
        if(empty($group)){
            ...
            $select = new Select('virtual_class');
            ...
            //$select->join($this->getPrefix().'course_categories','course_course_category.course_category_id=course_category.course_category_id',['category_name'=>'name']);
        ...
        switch($order){
            case 'asc':
                $select->order('topic asc');
                break;
            case 'desc':
                $select->order('topic desc');
                break;
            case 'recent':
        ...
        if(isset($filter))
        {
           ...
           $select->where("MATCH (topic,description,meeting_id) AGAINST ('$filter' IN NATURAL LANGUAGE MODE)");
        }
        ...
        //    $select->where("(end_date > {$time} OR end_date=0 )");
            $select->where("( (end_date IS NULL OR end_date=0 OR end_date > '$time') )");
