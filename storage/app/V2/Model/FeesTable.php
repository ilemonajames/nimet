...
class FeesTable extends BaseTable {
    ...
    public function getChildren($id){
        $categories = Fees::where('parent_id',$id)->orderBy('sort_order')->where('enabled',1)->get();
    ...
    public function getCategoriesForParent($parentId,$level=0,$exId=null){
        ...
        if(empty($parentId)){
            $categories = Fees::whereNull('parent_id')->orderBy('sort_order')->get();
        }
        else{
            $categories = Fees::where('parent_id',$parentId)->orderBy('sort_order')->get();
        }
