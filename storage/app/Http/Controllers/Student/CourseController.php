          ...
          //      $sessionContainer->url = selfURL();
                ...
                foreach($assignments as $assignment){
                    ...
                    if(!$assignmentSubmissionTable->hasSubmission($this->getId(),$assignment->assignment_id)){
                        ...
                        $message = __lang('homework-notification-mail',['homework'=>$assignment->title,'class'=>$previousClass->name,'link'=>$this->absoluteRoute('application/submit-assignment',['id'=>$assignment->assignment_id])]);
            ...
            if($next){
                ...
                foreach($allLectures as $row){
                    ...
                    if(!$sessionLogTable->hasAttendance($this->getId(),$session,$row->id)){
                        ...
                        return redirect()->route(MODULE.'.course.lecture',['lecture'=>$row->id,'sessionId'=>$session]);
                    }
