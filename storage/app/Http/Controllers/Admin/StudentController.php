...
use App\V2\Model\VirtualClassTable;
...
class StudentController extends Controller
{
    ...
    public function changepassword(Request $resquest, $id){
        $pass = $resquest->password;
        $save = User::find($id);
        if($save){
         $pass1 =  Hash::make($pass);  
        $save->password = $pass1;
        $save->save();
        session()->flash('flash_message','Student password changed successfully');
        return redirect()->back();
        }else{
          session()->flash('flash_message','Unable to change this student password');
        return redirect()->back();
        }
    ...
    public function view(Request $request,$id)
    {
        ...
        $session = $studentTable->getStudentSession($row->user_id);
        $semester = $studentTable->getStudentSemester($row->user_id);
    ...
    public function removestudentolevelresult($id){
    ...
    public function edit(Request $request,$id)
    {
        ...
        if ($request->isMethod('post')) {
           ...
           // $userData['first_name'] = $userData['name'];
            ...
            $data1['academic_session_id'] = $user->session;
            //}
            $academicSessionTable = new AcademicSessionTable();
            $rowset = $academicSessionTable->getAcademicSessionRecords($user->session);
            ...
            $data2['semester_id'] = "";
            //foreach($user as $groupRow){
                $data2['semester_id'] = $user->semester;
            //}
            $semesterTable = new SemesterTable();
            $rowset2 = $semesterTable->getSemesterRecords($user->semester);
    ...
    public function virtualclasses(Request $request){
        ...
        $paginator->setCurrentPageNumber((int)$request->get('page', 1));
        $paginator->setItemCountPerPage(30);
        return view('admin.virtualclasses.live',array(
            'paginator'=>$paginator,
            'pageTitle'=>__lang('virtual-classes').'  ('.$totalRecords.')',
            'attendanceTable'=>$attendanceTable,
    ...
    public function startclass($id){
        return view('admin.virtualclasses.start',array(
            'meeting_id'=>$id
        ));
    }
    ...
    public function addvirtualclasses(Request $request){
        $type = 'v';
        $table = new VirtualClassTable();
        $output = array();
        $output['id']=0;
        ...
        if($request->isMethod('post')){
            ...
            $form->setData($formData);
                ...
                if(empty($request->session_date)){
                    $date = Carbon::now()->toDateString();
                }
                else{
                    $date = Carbon::parse($request->session_date)->toDateString();
                }

                if(empty($request->session_end_date)){
                    $endDate = Carbon::now()->toDateString();
                }
                else{
                    $endDate = Carbon::parse($request->session_end_date)->toDateString();
                }
        ...
        $meetingData = [
            ...
            'start_time' => $request->session_date,
        ...
        $output =  $user->meetings()->save($meeting);
                ...
                $courseRow = VirtualClass::create($dbData);
                ...
                session()->flash('flash_message',__lang('new-virtual-class-addedd'));
                return adminRedirect(['controller'=>'student','action'=>'virtualclasses','id'=>$sessionId]);
        ...
        $output['action'] = 'add';
        $output['type'] = $type;
        $output['pageTitle'] = __lang('add-virtual-class');
        $output['lessonGroupTable'] = new LessonToLessonGroupTable();
        return view('admin.virtualclasses.new',$output);
    }
...
/*            $rowset = $sessionLessonTable->getSessionRecords($id);
    ...
    public function paymentHistory(Request $request){
        ...
        $invoices= PaymentHistory::with('user')->orderBy('id','desc')->paginate(20);
        

        $output = [
          'pageTitle'=>__lang('payment-history'),
            'paginator'=>$invoices
        ...
        for($i=1;$i<=$totalPages;$i++){
            ...
            foreach ($paginator as $row){
                ...
                foreach($fields as $field){
                    $fieldRow = $studentFieldsTable->getStudentFieldRecord($row->student_id,$field->student_field_id);
                    if(empty($fieldRow)){
                        $csvData[] ='';
                    }
    ...
    public function attendanceimport(Request $request){
        ...
        if($request->isMethod('post'))
        {
            ...
            if(!empty($failure)){
                $output['flash_message'] .= " $failure ".__("records failed");
            }
    ...
    public function massenroll(Request $request){
        ...
        if($request->isMethod('post'))
        {
            ...
            foreach($all_rows as $value){
                ...
                try{
                    if(!$studentTable->emailExists($dbData['email'])){
                        ...
                        $enrollData = array('student_id'=>$row->student_id,'course_id'=>$sessionId);
                        $studentSessionTable->addRecord($enrollData);
                    }
            ...
            $output['flash_message'] = __("you-enrolled-total",['total'=>$total]);
            if(!empty($failure)){
...
/*
    ...
    public function manageinstructors(Request $request,Course $course,Lesson $lesson){
        ...
        $sessionLessonTable = new SessionLessonTable();
        $sessionLessonAccountTable = new SessionLessonAccountTable();
        $row = $course->lessons()->where('id',$lesson)->first();
        //get accountlist
    ...
    public function import(Request $request){
        ...
        if($request->isMethod('post'))
        {
            ...
            $output['flash_message'] = __("import-success",['total'=>$total]);
            if(!empty($failure)){
                $output['flash_message'] .= " $failure ".__("records failed");
            }
