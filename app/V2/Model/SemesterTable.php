...
class SemesterTable extends BaseTable {
    ...
    public function getSemesterRecords($id){
	...
	public function getActiveSemesterRecords($id){
        $select = new Select($this->tableName);
        $select->where('id', $id);
        $select->where('enabled', 1);
        //$select->join($this->getPrefix().'admins',$this->tableName.'.admin_id='.$this->getPrefix().'admins.id',['about']);
    ...
    public function getChildren($id){
        $categories = Semester::where('parent_id',$id)->orderBy('sort_order')->where('enabled',1)->get();
    ...
    public function getCategoriesForParent($parentId,$level=0,$exId=null){
        ...
        if(empty($parentId)){
            $categories = Semester::whereNull('parent_id')->orderBy('sort_order')->get();
        }
        else{
            $categories = Semester::where('parent_id',$parentId)->orderBy('sort_order')->get();
        }
