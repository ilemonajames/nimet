...
class SemesterController extends Controller
{
    ...
    private function getGroupForm(){
        ...
        $categories = $table->getAllCategories();
        $form = new BaseForm();
        $form->createText('semester_name','Semester Name',true);
    ...
    public function addsemester(Request $request){
        ...
        if (request()->isMethod('post')) {
            ...
            $form->setData($data);
            if ($form->isValid()) {
                 $find = Semester::where('enabled',1)->first();
			If(!empty($find->enabled)){
				If($find->enabled == $request->enabled){
				session()->flash('flash_message',"You can not enable more than one semester at a time. Please disable the active semester first.");
                return adminRedirect(array('controller'=>'semester','action'=>'index'));
			}
			}
                ...
                session()->flash('flash_message',__lang('semester-created'));
                return adminRedirect(array('controller'=>'semester','action'=>'index'));
        ...
        $output['form'] = $form;
        $output['pageTitle']= __lang('add-semester');
        $output['action']='addsemester';
        $output['id']=null;
    ...
    public function editsemester(Request $request,$id){
        ...
        $output['form'] = $form;
        $output['id'] = $id;
        $output['pageTitle']= __lang('edit-semester');
        $output['row']= $row;
        $output['action']='editsemester';

        $viewModel = viewModel('admin',__CLASS__,'addsemester',$output);
        return $viewModel ;
    }
