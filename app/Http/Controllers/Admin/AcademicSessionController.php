...
class AcademicSessionController extends Controller
{
    ...
    public function session(Request $request){
        ...
        $table = new AcademicSessionTable();
        ...
        return viewModel('admin',__CLASS__,__FUNCTION__,array(
            ...
            'pageTitle'=>__lang('academic-session'),
    ...
    private function getGroupForm(){
        ...
        $categories = $table->getAllCategories();
        $form = new BaseForm();
        $form->createText('session_name','Academic Session Name',true);
    ...
    public function addsession(Request $request){
        ...
        if (request()->isMethod('post')) {
            ...
            if ($form->isValid()) {
               $find = AcademicSession::where('enabled',1)->first();
			If(!empty($find->enabled)){
				If($find->enabled == $request->enabled){
				session()->flash('flash_message',"You can not enable more than one academic session at a time. Please disable the active session first.");
                return adminRedirect(array('controller'=>'academic','action'=>'session'));
			}
			}
			   
			   //$session = Setting::where('id',109)->first();$session->value
                $array = $form->getData();
                ...
                session()->flash('flash_message',__lang('academic-session-created'));
                return adminRedirect(array('controller'=>'academic','action'=>'session'));
        ...
        $output['form'] = $form;
        $output['pageTitle']= __lang('add-academic-session');
        $output['action']='addsession';
        $output['id']=null;
    ...
    public function editsession(Request $request,$id){
        ...
        $output['row']= $row;
        $output['action']='editsession';

        $viewModel = viewModel('admin',__CLASS__,'addsession',$output);
        return $viewModel ;
    }

    public function defaultsession(Request $request,$id){
        
        try{
            $table = DB::select("UPDATE `settings` SET `value` = '$id' WHERE `settings`.`id` = 109"); 
            flashMessage(__lang('default-session-updated'));
        }
        catch(\Exception $ex){
