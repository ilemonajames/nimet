...
class RegisterController extends Controller
{
    ...
    */
      ...
      /*  if(setting('regis_captcha_type')=='google'){
       ...
       //$user = $this->create($request->all());
        ...
        //if ($response = $this->registered($request, $user)) {
            //return $response;
            return redirect('/aptitude/index/'.$user->id);
        //}
    ...
    protected function create(array $data)
    {
        ...
        $std = $user->student()->create([
           'mobile_number'=>$data['mobile_number']
        ]);
         
        $studentSessionTable = new StudentSessionTable();
        //$select = DB::table('course_course_category')->where('course_category_id', $data['course_of_study'])->get();
		$select = DB::table('course_course_category')
                    ->join('courses', 'course_course_category.course_id', '=', 'courses.id')
					->where('course_category_id', $data['course_of_study'])
					->get();
        ...
        $message .= "<br/><br/>You can access your test link here at any time <br/>";
        $message .= "<a href='".url('/aptitude/index/'.$user->id)."'>Test Link</a>";
    ...
    public function showRegistrationForm()
    {
          ...
          $academicSessions = AcademicSession::where('enabled',1)->get();
          $semesters = Semester::where('enabled',1)->get();
          $fields = StudentField::orderBy('sort_order')->where('enabled',1)->get();
          return tview('auth.register',compact('enableRegistration','fields','academicSessions','semesters'));
    ...
    public function confirm(){
        ...
        /* $select = DB::table('course_course_category')->where('course_category_id', 3)->get();
