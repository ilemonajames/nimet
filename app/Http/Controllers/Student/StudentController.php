...
use App\V2\Model\AcademicSessionTable;
use App\V2\Model\SemesterTable;
...
class StudentController extends Controller {
    ...
    public function mysessions(Request $request){
        ...
        $studentsTable1 = new StudentTable();
        $id = $this->getId();
        $row1 = $studentsTable1->getRecord($id);
        $user1  = User::find($row1->user_id);
        $session_value = $user1->session;
        $semester_value = $user1->semester;

        $session = Setting::where('id',109)->first();
		...
		$find1 = AcademicSession::where('enabled',1)->first();
		 $asid = $find1->id;
		 $find2 = Semester::where('enabled',1)->first();
		 $sid = $find2->id;
		
        $paginator = $studentSessionTable->getCurrentStudentRecords(true,$studentId,$asid,$sid);
        ...
        $output = [
          'pageTitle'=>__lang('Enrolled Courses'),
            ...
            'payment_status'=>$user1->payment_status,
    ...
    public function receipt($id){
        ...
        $row = $studentsTable->getRecord($id1);

        $user1  = User::find($row->user_id);
        $session_value = $user1->session;
        $semester_value = $user1->semester;

        $session1 = AcademicSession::find($session_value);
        $semester1 = Semester::find($semester_value);

        $this->data['session_name'] = $session1->session_name;
        $this->data['semester_name'] = $semester1->semester_name;

        $general_address = Setting::where('key','general_address')->first();

        $fee_name = DB::select("select * from payment_histories p, fees f where p.fee_id=f.id"); 

        $invoices= PaymentHistory::find($id);
        $this->data['pageTitle'] = __lang('My Reciept');
        $this->data['receipt'] = $invoices;
        $this->data['fee_name'] = $fee_name;
        $this->data['general_address'] = $general_address;
        $this->data['user'] = $row;
        return view('student.student.receipt',$this->data);
    }
    ...
    public function myfees(Request $request){
        ...
        $fees = Fees::where('enabled', 1)->orderBy('id','desc')->get();
        //$fees = $fee->amount;
        $id = $this->getId();
        $studentsTable = new StudentTable();
        ...
        if ($request->isMethod('post')) {
            ...
            $pay->user_id = $user_id;
            $pay->payment_reference = $paymentReference;
            $pay->processor_id = $processorId;
            $pay->transaction_id = $transactionId;
            $pay->amount = $amount;
            $pay->total_amount = $total_amount;
            $pay->session_id = $session_id;
            $pay->semester_id = $semester_id;
            $pay->payment_status = $payment_status;
            $pay->narration = $narration;
            //$pay->fee_id = $fee->id;
            $pay->payment_method = __lang('online');
            $pay->currency_id = currentCurrency()->id;
            $pay->save();
            ...
            $user1->payment_status = 1;
            $user1->save();

            flashMessage(__lang('payment-successful'));
            }else{
            flashMessage(__lang('payment-unsuccessful'));    
            }
            return back();
        ...
        $paymentMethod = PaymentMethod::where('directory','bank')->first();
        $settings = $paymentMethod->details;

        

        $id = Auth::user()->id;
        $invoices= PaymentHistory::where('user_id',$id)->orderBy('id','desc')->paginate(20);
        $user = Auth::user();
        

        $output = [
          'pageTitle'=>__lang('fee-payments'),
            'paginator'=>$invoices,
            'id'=>$studentId,
            'user'=>$user,
            'fees'=>$fees,
    ...
    public function mysubscriptions(Request $request){
        ...
        $fees = Package::where('enabled', 1)->orderBy('id','desc')->get();
        //$fees = $fee->amount;
        $id = $this->getId();
        $studentsTable = new StudentTable();
        $row = $studentsTable->getRecord($id);
        ...
        if ($request->isMethod('post')) {
            ...
            flashMessage(('Subscription payment successful'));
            }else{
            flashMessage(('Subscription payment unsuccessful'));    
            }
            return back();
        ...
        $id = Auth::user()->id;
        $invoices= Subscription::where('user_id',$id)->orderBy('id','desc')->paginate(20);
        $user = Auth::user();
        

        $output = [
          'pageTitle'=> ('Subscribe for NIMET E-Library'),
            'paginator'=>$invoices,
            'id'=>$studentId,
            'user'=>$user,
            'fees'=>$fees,
    ...
    public function play(Request $request,$id){
        ...
        //$this->data['videoUrl'] = studentUrl(['controller'=>'student','action'=>'serve','id'=>$video->id]);
        $this->data['videoUrl'] = route('student.student.serve', $video->id, '');
        $poster = '';
        ...
        $viewModel = viewModel('student',__CLASS__,__FUNCTION__,$this->data);
    ...
    public function elibrary(Request $request)
    {
            ...
            $sub = Subscription::where('end_date','>', date('Y-m-d'))->where('user_id',Auth::user()->id)->first();

        $output = [
            'pageTitle'=> ('NIMET E-Library'),
            'paginator'=>$paginator,
            'filter'=>$filter,
            'text'=>$text,
            'sub'=>$sub,
        ...
        if ($request->isMethod('post')) {
            ...
            if ($form->isValid()) {
                ...
                if(!empty($_FILES['picture']['name'])){
                    ...
                    $this->makeUploadDir();
                    rename($_FILES['picture']['tmp_name'],$newPath);
                    $array['picture'] = $newPath;

                }

                if(!empty($_FILES['olevel_result']['name'])){
                    @unlink($row->olevel_result);

                    $file1 = $_FILES['olevel_result']['name'];
                    $newPath1 = $this->uploadDir.'/'.time().$id.'_'.sanitize($file1);
                    $this->makeUploadDir();
                    rename($_FILES['olevel_result']['tmp_name'],$newPath1);
                    $array['olevel_result'] = $newPath1;
            ...
            $data1['academic_session_id'] = "";
            //foreach($user as $groupRow){
            $data1['academic_session_id'] = $row->session;
            //}
            $academicSessionTable = new AcademicSessionTable();
            $rowset = $academicSessionTable->getActiveAcademicSessionRecords($row->session);
            foreach($rowset as $groupRow1){
                $data1['academic_session_id'] = $groupRow1->id;
            }
            ...
            $data2['semester_id'] = "";
            //foreach($user as $groupRow){
                $data2['semester_id'] = $row->semester;
            //}
            $semesterTable = new SemesterTable();
            $rowset2 = $semesterTable->getActiveSemesterRecords($row->semester);
            foreach($rowset2 as $groupRow2){
                $data2['semester_id'] = $groupRow2->id;
            }
    ...
    public function removeolevelresult(Request $request){

	    $user = Auth::user();
	    $user->olevel_result = null;
	    $user->save();

        flashMessage(__lang('olevel-result-removed'));
        return back();
    }
