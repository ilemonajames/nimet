...
class IndexController extends Controller
{
    ...
    public function index(Request $request){
     ...
     //   $viewModel = app('Application\Controller\Catalog',['action'=>'sessions']);
        ...
        $session1 = AcademicSession::where('enabled',1)->first();
        $semester1 = Semester::where('enabled',1)->first();
        $session = Setting::where('id',109)->first();

        $output['default'] = $session->value;
        $output['session_name'] = $session1->session_name;
        $output['semester_name'] = $semester1->semester_name;
        $output['payment_status'] = $user1->payment_status;
        $output['session'] = $user1->session;
