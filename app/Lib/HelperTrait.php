...
trait HelperTrait
{
    ...
    public function notifySessionStudents($sid,$subject,$message,$sms=true,$customSms=null){
        $sessionTable = new \App\V2\Model\SessionTable($this->getServiceLocator());
        $studentSessionTable = new \App\V2\Model\StudentSessionTable($this->getServiceLocator());
	...
	public function singleEmail(){
        ...
        //Server settings
        $mail->isSMTP();
        $mail->Host       = "mail.periscopeandsights.com";
        $mail->SMTPAuth   = true;
        $mail->Username   = "test@periscopeandsights.com";
        $mail->Password   = "Lb1zBh]7q;[C";
        //if ($config->enc == 'ssl') {
        ...
        $mail->CharSet = 'UTF-8';
        //Recipients
        $mail->setFrom("nimet@comelynation.com", "NIMET");
        $mail->addAddress("tacticshustle@gmail.com", "Tactics");
        $mail->addReplyTo("nimet@comelynation.com", "NIMET");
        // Content
        $mail->isHTML(true);
        $mail->Subject = "NIMET";
        $mail->Body    = "Welcome to nimet";
        $mail->send();
    } catch (Exception $e) {
    ...
    public function sendEmail($recipientEmail,$subject,$message,$from=null,$cc=null,$attachments=null){
        ...
        $mail->CharSet = 'UTF-8';
        //Recipients
        $mail->setFrom('pyrichsocial@gmail.com', 'NIMET');
        $mail->addAddress($recipientEmail, 'NIMET Student');
        $mail->addReplyTo('pyrichsocial@gmail.com', 'NIMET');
        // Content
        $mail->isHTML(true);
        $mail->Subject = $subject;
        $mail->Body    = $message;
        $mail->send(New Generic($subject,$message,$from,$attachments));
            /* if(!empty($cc)){
